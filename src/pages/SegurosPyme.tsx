...
const SegurosPyme = () => {
	const handleSelectPlan = (planIndex: number) => {
		...
		toast.success(`Plan ${planName} seleccionado`, {
			description:
				'Serás redirigido a WhatsApp para completar tu cotización empresarial',
		});
		setTimeout(() => {
	...
	const genericWhatsappMessage =
		'Hola, me gustaría obtener más información sobre los seguros para PYME.';
							...
							<div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
								<div>
									...
									<p className='text-sm text-muted-foreground'>
										Empieza a regir desde el primero día del mes seguiente a la
										contratación
									</p>
								</div>
								<div>
									...
									<p className='text-sm text-muted-foreground'>
										Atención prioritaria para tu empresa
									</p>
								</div>
								<div>
									<p className='font-semibold mb-2'>Planes Flexibles</p>
									<p className='text-sm text-muted-foreground'>
										Adaptamos la cobertura a tus necesidades
									</p>
								</div>
							</div>
							<a
